...
struct counter_array_pit_config {
    PIT_Type *base;
    struct counter_config_info info;
    bool enable_in_debug;
};

struct pit_alarm_data {
    struct device *dev;
...
struct counter_array_pit_data {
    struct pit_alarm_data alarm_data[MAX_CHANNELS];
};

static void pit_irq_handler(void *arg)
{
    struct pit_alarm_data *data = (struct pit_alarm_data *)arg;
    const struct counter_array_pit_config *config = data->dev->config_info;

    PIT_ClearStatusFlags(config->base, data->id, kPIT_TimerFlag);

    if (data->callback) {
        uint32_t ticks = PIT_GetCurrentTimerCount(config->base, data->id);
        data->callback(data->dev, data->id, ticks, data->user_data);
    }
...
static int counter_array_mcux_pit_start(struct device *dev, uint8_t id)
{
    if (id >= MAX_CHANNELS) {
        LOG_ERR("Invalid counter id");
        return -ENOTSUP;
    }

    const struct counter_array_pit_config *config = dev->config_info;
    PIT_EnableInterrupts(config->base, id, kPIT_TimerInterruptEnable);
    PIT_StartTimer(config->base, id);

    return 0;
}

static int counter_array_mcux_pit_stop(struct device *dev, uint8_t id)
{
    if (id >= MAX_CHANNELS) {
        LOG_ERR("Invalid counter id");
        return -ENOTSUP;
    ...
    const struct counter_array_pit_config *config = dev->config_info;
    PIT_DisableInterrupts(config->base, id, kPIT_TimerInterruptEnable);
    PIT_StopTimer(config->base, id);

    return 0;
}

static int counter_array_mcux_pit_get_value(const struct device *dev, uint8_t id, uint32_t *ticks)
{
    if (id >= MAX_CHANNELS) {
        LOG_ERR("Invalid counter id");
        return -ENOTSUP;
    ...
    const struct counter_array_pit_config *config = dev->config_info;
    *ticks = PIT_GetCurrentTimerCount(config->base, id);
    return 0;
}

static int counter_array_mcux_pit_set_alarm(const struct device *dev, uint8_t id, 
                                            const struct counter_alarm_cfg *alarm_cfg)
{
    ...
    const struct counter_array_pit_config *config = dev->config_info;
    struct counter_array_pit_data *data = dev->driver_data;

    PIT_StopTimer(config->base, id);
    PIT_SetTimerPeriod(config->base, id, alarm_cfg->ticks);
    data->alarm_data[id].callback = alarm_cfg->callback;
    data->alarm_data[id].user_data = alarm_cfg->user_data;
    data->alarm_data[id].dev = (struct device *)dev;
    data->alarm_data[id].id = id;
    PIT_EnableInterrupts(config->base, id, kPIT_TimerInterruptEnable);
    PIT_StartTimer(config->base, id);
    return 0;
}

static int counter_array_mcux_pit_cancel_alarm(const struct device *dev, uint8_t id)
{
    if (id >= MAX_CHANNELS) {
        LOG_ERR("Invalid counter id");
        return -ENOTSUP;
...
static int counter_array_mcux_pit_set_top_value(const struct device *dev, uint8_t id, 
