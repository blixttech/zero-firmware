...
#define BCB_GPIO_DEV_PIN(node_label, pin_name)                                                     \
	DT_PHA_BY_NAME(DT_NODELABEL(node_label), gpios, pin_name, pin)
#define BCB_GPIO_DEV_PIN_FLAGS(node_label, pin_name)                                               \
	DT_PHA_BY_NAME(DT_NODELABEL(node_label), gpios, pin_name, flags)

#define BCB_INIT_GPIO_PIN(node_label, pin_name, pin_flags)                                         \
	do {                                                                                       \
		...
		gpio_pin_configure(BCB_GPIO_DEV(pin_name), BCB_GPIO_DEV_PIN(node_label, pin_name), \
				   (pin_flags));                                                   \
	} while (0)

#define BCB_SET_GPIO_PIN(node_label, pin_name, value)                                              \
	gpio_pin_set_raw(BCB_GPIO_DEV(pin_name), BCB_GPIO_DEV_PIN(node_label, pin_name), (value))

#define BCB_GET_GPIO_PIN(node_label, pin_name)                                                     \
	gpio_pin_get_raw(BCB_GPIO_DEV(pin_name), BCB_GPIO_DEV_PIN(node_label, pin_name))
...
#define BCB_INIT_IC(node_label, ch_name)                                                           \
	do {                                                                                       \
...
#define BCB_OCPT_ADJ_PWM_DEV_LABEL(node_label, ch_name)                                            \
	DT_LABEL(DT_PHANDLE_BY_NAME(DT_NODELABEL(node_label), pwms, ch_name))
#define BCB_OCPT_ADJ_PWM_CHANNEL(node_label, ch_name)                                              \
	DT_PHA_BY_NAME(DT_NODELABEL(node_label), pwms, ch_name, channel)
#define BCB_OCPT_ADJ_PWM_PERIOD(node_label, ch_name)                                               \
	DT_PHA_BY_NAME(DT_NODELABEL(node_label), pwms, ch_name, period)

#define BCB_INIT_OCPT_ADJ_PWM(node_label, ch_name)                                                 \
	do {                                                                                       \
		BCB_OCPT_ADJ_PWM_DEV(ch_name) =                                                    \
			device_get_binding(BCB_OCPT_ADJ_PWM_DEV_LABEL(node_label, ch_name));       \
...
#define BCB_SET_OCPT_ADJ_PWM(node_label, ch_name, dcycle)                                          \
	pwm_pin_set_cycles(BCB_OCPT_ADJ_PWM_DEV(ch_name),                                          \
			   BCB_OCPT_ADJ_PWM_CHANNEL(node_label, ch_name),                          \
			   BCB_OCPT_ADJ_PWM_PERIOD(node_label, ch_name),                           \
			   (BCB_OCPT_ADJ_PWM_PERIOD(node_label, ch_name) * (dcycle) / 100), 0);

/* ---------------- Overcurrent protection limit related macros ---------------- */
#define BCB_OCPL_ADJ_DAC_DEV(ch_name)            (bcb_ctrl_data.dac_##ch_name)
#define BCB_OCPL_ADJ_DAC_DEV_LABEL(node_label, ch_name)                                            \
	DT_LABEL(DT_PHANDLE_BY_NAME(DT_NODELABEL(node_label), io_channels, ch_name))
#define BCB_OCPL_ADJ_DAC_CHANNEL(node_label, ch_name)                                              \
	DT_PHA_BY_NAME(DT_NODELABEL(node_label), io_channels, ch_name, output)

#define BCB_INIT_OCPL_ADJ_DAC(node_label, ch_name)                                                 \
	do {                                                                                       \
		BCB_OCPL_ADJ_DAC_DEV(ch_name) =                                                    \
			device_get_binding(BCB_OCPL_ADJ_DAC_DEV_LABEL(node_label, ch_name));       \
		if (BCB_OCPL_ADJ_DAC_DEV(ch_name) == NULL) {                                       \
			LOG_ERR("Could not get DAC device");                                       \
			return -EINVAL;                                                            \
		}                                                                                  \
		struct dac_channel_cfg cfg = { .channel_id = BCB_OCPL_ADJ_DAC_CHANNEL(node_label,  \
										      ch_name),    \
					       .resolution = 12 };                                 \
		dac_channel_setup(BCB_OCPL_ADJ_DAC_DEV(ch_name), &cfg);                            \
	} while (0)

#define BCB_SET_OCPL_ADJ_DAC(node_label, ch_name, value)                                           \
	do {                                                                                       \
		dac_write_value(BCB_OCPL_ADJ_DAC_DEV(ch_name),                                     \
				BCB_OCPL_ADJ_DAC_CHANNEL(node_label, ch_name), value);             \
	} while (0)
...
struct bcb_ctrl_data {
    ...
    struct gpio_callback on_off_cb_data;
};
...
static void on_off_status_changed(struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	...
	if (on_off_status) {
		...
		if (BCB_IC_ONOFF_ETIME_COMP_HT((uint64_t)(UINT16_MAX - BCB_IC_ONOFF_TICKS_GUARD),
					       onoff_duration)) {
			/* We can use the input capture timer to measure the duration between ON and OFF events
             		 * more accurately.
             		 */
			onoff_duration = (ic_off < ic_on) ?
						 ((uint32_t)UINT16_MAX - ic_on + ic_off) :
						 (ic_off - ic_on);
		} else {
		...
		if (on_off) {
			...
			if (bcb_ctrl_data.ocpt_active) {
				...
				if (bcb_ctrl_data.ocpt_callback) {
					...
					uint32_t ocpt_start;
					if (bcb_ctrl_data.ocpt_direction ==
					    BCB_OCP_TEST_TGR_DIR_P) {
						ocpt_start =
							BCB_GET_IC_VALUE(itimestamp, ocp_test_tr_p);
					} else {
						ocpt_start =
							BCB_GET_IC_VALUE(itimestamp, ocp_test_tr_n);
					}
					uint64_t ocpt_duration = (ic_off < ocpt_start) ?
									 ((uint32_t)UINT16_MAX -
									  ocpt_start + ic_off) :
									 (ic_off - ocpt_start);
					bcb_ctrl_data.ocpt_callback(ocpt_duration,
								    bcb_ctrl_data.ocpt_direction);
				}
			} else {
...
static int bcb_ctrl_init()
{
	...
	gpio_pin_interrupt_configure(BCB_GPIO_DEV(on_off_status),
				     ...
				     GPIO_INT_EDGE_BOTH);
	gpio_init_callback(&bcb_ctrl_data.on_off_cb_data, on_off_status_changed,
			   BIT(DT_PHA_BY_NAME(DT_NODELABEL(dctrl), gpios, on_off_status, pin)));
	gpio_add_callback(BCB_GPIO_DEV(on_off_status), &bcb_ctrl_data.on_off_cb_data);
	...
	/* Configure overcurrent protection test timestamping */
