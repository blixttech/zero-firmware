...
static inline void dma_mcux_set_reload(const struct dma_config *config, edma_tcd_t *tcd)
{
	const struct dma_block_config *block_config = config->head_block;
	int32_t src_offset;
	int32_t dst_offset;

	switch (config->channel_direction) {
	case MEMORY_TO_MEMORY:
		src_offset = -1 * block_config->block_size;
		dst_offset = -1 * block_config->block_size;
		break;
	...
	if (block_config->source_reload_en) {
		tcd->SLAST = src_offset;
	}

	if (block_config->dest_reload_en) {
		tcd->DLAST_SGA = dst_offset;
	}
}

static inline void dma_mcux_set_cyclic(edma_handle_t *handle)
{
	int i;
	for (i = 0; i < handle->tcdUsed; i++) {
		handle->tcdPool[i].CSR |= DMA_CSR_ESG_MASK;
		handle->tcdPool[i].CSR &= ~(uint16_t)DMA_CSR_DREQ_MASK;
	}
}
				   ...
				   struct dma_config *config)
{
	...
	if (block_config->source_gather_en || block_config->dest_scatter_en) {
		...
		dma_mcux_set_reload(config, tcdRegs);

		if (!config->dma_callback) {
			EDMA_DisableChannelInterrupts(DEV_BASE(dev), channel,
